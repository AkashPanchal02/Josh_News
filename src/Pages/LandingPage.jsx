...
const LandingPage = () => {
    ...
    useEffect(() => {
            const fetchNews = async () => {
                ...
                setResult({articles: []})
                try {
                    const response = await fetch(`https://newsapi.org/v2/everything?q=latest&apiKey=${import.meta.env.VITE_NEWSAPI_KEY}`)
                    if (!response.ok) {
  ...
  return (
    <div>
        <section className={`conatiner ${isDark? "bg-black": "bg-blue-50"}  py-40 text-center`}>
        ...
        <div className="explore-new-categories">
            ...
            <div className="category-grid grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                {["general", "business", "entertainment", "science", "sports",  "political", "social", "health", "technology"].map((category) => (
                    <Link
        ...
        <div className="recent-headlines">
            <div className='container mx-auto'>
                ...
                <div className='grid md:grid-cols-2 lg:grid-cols-3 gap-6'>
                    {result.articles?.filter(({urlToImage}) => urlToImage)?.map(({ title, source, description, url, urlToImage, publishedAt }) => (
